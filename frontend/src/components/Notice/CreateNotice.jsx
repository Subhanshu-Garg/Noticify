...
import { MDBContainer, MDBRow, MDBCol, MDBBtn, MDBInput, MDBTypography, MDBFile, MDBTextArea } from "mdb-react-ui-kit";
...
const CreateNotice = ({ organizationId, organizationName }) => {
  ...
  const { loading: createLoading } = useSelector((state) => state.createNotice);
  ...
  return (
    <MDBContainer className="my-2">
      ...
      <MDBRow>
        <MDBCol>
          <form inert onSubmit={handleCreateNotice}>
            <MDBInput
              className="mb-2"
              label="Notice Title"
              value={title}
